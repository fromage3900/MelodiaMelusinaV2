"""Create WBP_MelodiaElementWheel — centered radial medallion using MedallionRosette.
Strictly presentation-only element wheel for the battle UI row, no logic."""
import json
import sys
...
ASSET = "/Game/Melodia/UI/Foundation/WBP_MelodiaElementWheel"
TEXTURE = "/Game/Melodia/UI/Textures/Universal/T_Melodia_Universal_MedallionRosette"
...
def main():
    ...
    try:
        invoke("ui_query", action="add_widget", asset_path=ASSET, widget_class="Image",
               widget_name="ElementWheelImage", parent_name="CanvasRoot", compile=False)
    except RuntimeError:
        pass
    invoke("ui_query", action="set_anchor_preset", asset_path=ASSET, widget_name="ElementWheelImage",
           preset="center", compile=False)
    invoke("ui_query", action="set_slot_property", asset_path=ASSET, widget_name="ElementWheelImage",
           offsets={"left": -192, "top": -192, "right": 384, "bottom": 384}, z_order=0, compile=False)
    invoke("ui_query", action="set_brush", asset_path=ASSET, widget_name="ElementWheelImage",
           property_name="Brush", texture_path=TEXTURE, draw_type="Image", tint_color="#F6C95FFF",
           compile=False)
    invoke("ui_query", action="set_widget_is_variable", asset_path=ASSET, widget_name="ElementWheelImage",
    ...
    return {"status": "authored", "asset": ASSET, "errors": crc.get("error_count", 0), "widgets": tree2.get("widget_count")}
