...
@pytest.fixture(scope="module")
def currencies(registry: dict) -> list[dict]:
    rows = registry.get("currencies")
    assert isinstance(rows, list) and rows, "registry declares no currencies"
    return rows


def test_schema_and_version(registry: dict) -> None:
    assert registry["schema"] == "melodia_currency_registry.v1"
    assert registry["registry_schema_version"] >= 1


def test_currency_ids_are_unique(currencies: list[dict]) -> None:
    ...
    ids = [row["currency_id"] for row in currencies]
    duplicates = sorted({i for i in ids if ids.count(i) > 1})
    assert not duplicates, f"duplicate currency_id(s): {duplicates}"
    assert all(ids), "a row has an empty currency_id and could never be granted or spent"


def test_seed_currencies_present(currencies: list[dict]) -> None:
...
def test_kinds_are_valid(currencies: list[dict]) -> None:
    for row in currencies:
        assert row["kind"] in VALID_KINDS, f"{row['currency_id']}: bad kind {row['kind']!r}"


def test_resource_currencies_have_a_positive_cap(currencies: list[dict]) -> None:
    # A Resource with max_value 0 clamps to zero, so every grant is a silent no-op.
    for row in currencies:
        if row["kind"] == "Resource":
            assert row.get("max_value", 0) > 0, (
                f"{row['currency_id']}: Resource with max_value {row.get('max_value')} clamps "
                "to zero and can never hold a balance"
            )


def test_non_resource_currencies_do_not_set_a_cap(currencies: list[dict]) -> None:
...
def test_defaults_are_within_their_cap(currencies: list[dict]) -> None:
    for row in currencies:
        if row["kind"] == "Resource":
            assert row.get("default_value", 0) <= row["max_value"], (
                f"{row['currency_id']}: starts above its cap and is clamped down on first load"
            )


def test_legacy_element_mapping_is_one_to_one(currencies: list[dict]) -> None:
    ...
    for row in currencies:
        ...
        assert element in LEGACY_ELEMENTS, f"{row['currency_id']}: bad element {element!r}"
        assert element not in claimed, (
            f"{element} is claimed by both {claimed[element]} and {row['currency_id']}"
        )
        claimed[element] = row["currency_id"]

    assert set(claimed) == LEGACY_ELEMENTS, (
        f"unclaimed legacy elements: {sorted(LEGACY_ELEMENTS - set(claimed))}"
    )


def test_mana_and_golden_do_not_inflate_total_collected(currencies: list[dict]) -> None:
    ...
    by_id = {row["currency_id"]: row for row in currencies}
    for currency_id in ("Golden", "Mana"):
        assert by_id[currency_id]["counts_toward_total_collected"] is False


def test_golden_is_the_refundable_currency(currencies: list[dict]) -> None:
    refundable = {row["currency_id"] for row in currencies if row.get("refundable")}
    assert "Golden" in refundable, (
        "Golden must stay refundable: the wardrobe's purchase path calls TryRefundGolden "
        "to unwind a failed cosmetic grant."
    )


def test_token_catalog_variants_resolve_to_registry_currencies() -> None:
    ...
    """
    assert CATALOG_JSON.exists(), f"{CATALOG_JSON} is missing; re-export it"
...
def test_python_mirror_agrees_with_the_export() -> None:
    ...
    assert str(CATALOG_JSON) == TOKEN_SOURCE, (
    ...
    assert {k: v["element"] for k, v in TOKEN_TYPES.items()} == exported
