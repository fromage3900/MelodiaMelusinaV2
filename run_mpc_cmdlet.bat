...
set "UE_EXE=%UE_ROOT%\Engine\Binaries\Win64\UnrealEditor-Cmd.exe"
if not exist "%UE_EXE%" (
    echo Unreal Engine 5.8 not found at "%UE_EXE%".
    echo Set MELODIA_UNREAL_ROOT for this workstation.
    exit /b 1
)
"%UE_EXE%" "%ROOT%BS_GodFile.uproject" -run=pythonscript -script="%ROOT%Content\Python\create_tp_melusina_mpc.py"
endlocal
