...
public:
	...
	/**
	 * Fully stop the server: signals the worker thread, joins it, and tears
	 * down the listener socket.
	 *
	 * Call this instead of Stop(). Stop() is the FRunnable signal hook and is
	 * invoked by FRunnableThread's destructor -- doing the thread teardown
	 * there recurses infinitely.
	 */
	void Shutdown();
	...
	/** Signal-only: asks the worker loop to exit. Does NOT join or delete the thread. */
