...
public class VRM4ULoader : ModuleRules
{
	...
	public VRM4ULoader(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		// VrmConvertModel.cpp and VrmConvertModel_Description.cpp each define the same
		// anonymous-namespace helpers (BoneMapOpt, GetBoneNodeFromMeshID, FindMesh, ...).
		// Internal linkage makes that legal across separate translation units, but a unity
		// build concatenates them into one TU and every symbol collides. Upstream issue on
		// UE 5.8; disabling unity for this module only is the non-invasive fix.
		bUseUnity = false;
