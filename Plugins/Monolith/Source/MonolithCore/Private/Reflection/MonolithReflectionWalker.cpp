	...
	FBulkFillFieldWrite& OutWrite)
{
	...
	// MUST precede the FObjectProperty branch below: FClassProperty derives from
	// FObjectProperty, so without this a TSubclassOf<> field falls through to
	// WriteObjectRef with PropertyClass == UClass::StaticClass(), and
	// StaticLoadObject(UClass::StaticClass(), "/Game/Foo/BP_Bar") always returns
	// null because that path loads a UBlueprint, not its generated class.
	if (FClassProperty* ClassProp = CastField<FClassProperty>(Prop))
	{
		WriteClassRef(ClassProp, ValuePtr, JsonVal, OutWrite);
		return;
	}
...
// ---------------------------------------------------------------------------
// Class ref (TSubclassOf<> / UClass*).
//
// Accepts, in order of preference:
//   - a generated-class path        "/Game/Foo/BP_Bar.BP_Bar_C"
//   - a Blueprint asset path        "/Game/Foo/BP_Bar"      (the _C is appended)
//   - a native class path or name   "/Script/Engine.Actor", "Actor", "AActor"
//
// The _C retry mirrors the logic already proven in
// MonolithBlueprintInternal::ResolveDefaultObjectForPin, which had to solve the
// same problem on the pin side. Before this existed, every TSubclassOf write
// through set_cdo_property / set_property_at_path / set_cdo_properties /
// seed_data_asset failed for Blueprint class paths.
// ---------------------------------------------------------------------------
void FMonolithReflectionWalker::WriteClassRef(FClassProperty* ClassProp, void* ValuePtr, const TSharedPtr<FJsonValue>& JsonVal, FBulkFillFieldWrite& OutWrite)
{
	const FString PathStr = JsonVal->AsString();
	OutWrite.ProposedValue = PathStr;

	UObject* OldRef = ClassProp->GetObjectPropertyValue(ValuePtr);
	OutWrite.CurrentValue = OldRef ? OldRef->GetPathName() : FString();
	...
	if (PathStr.Contains(TEXT("/")))
	{
		if (UObject* Loaded = StaticLoadObject(UObject::StaticClass(), nullptr, *PathStr))
		{
			Resolved = Cast<UClass>(Loaded);
		}

		// '/Game/Foo/BP_Bar' loads the UBlueprint successfully -- wrong kind -- so a
		// bare null check is not enough to know the retry is needed.
		if (!Resolved && !PathStr.EndsWith(TEXT("_C")))
		{
			int32 LastSlash = INDEX_NONE;
			if (PathStr.FindLastChar(TEXT('/'), LastSlash))
			{
				const FString Leaf = PathStr.Mid(LastSlash + 1);
				const FString RetryPath = FString::Printf(TEXT("%s.%s_C"), *PathStr, *Leaf);
				if (UObject* Retry = StaticLoadObject(UObject::StaticClass(), nullptr, *RetryPath))
				{
					Resolved = Cast<UClass>(Retry);
				}
			}
		}
	}
	...
	{
		// Bare native name, tolerant of the A/U prefix convention.
		Resolved = FindFirstObject<UClass>(*PathStr, EFindFirstObjectOptions::None);
		if (!Resolved)
		{
			for (const TCHAR* Prefix : { TEXT("A"), TEXT("U"), TEXT("F") })
			{
				const FString Prefixed = FString(Prefix) + PathStr;
				Resolved = FindFirstObject<UClass>(*Prefixed, EFindFirstObjectOptions::None);
				if (Resolved) break;
			}
		}
	}
	...
	{
		OutWrite.bOk = false;
		OutWrite.Reason = FString::Printf(
			TEXT("class ref '%s' did not resolve to a UClass. For a Blueprint pass either "
				 "'/Game/Path/BP_Name' or '/Game/Path/BP_Name.BP_Name_C'; for a native class "
				 "pass '/Script/Module.ClassName' or the bare class name."),
			*PathStr);
		return;
	}

	// MetaClass is the TSubclassOf<> constraint. Enforce it here rather than letting
	// a wrong-hierarchy class land and fail later at load or compile.
	if (ClassProp->MetaClass && !Resolved->IsChildOf(ClassProp->MetaClass))
	{
		OutWrite.bOk = false;
		OutWrite.Reason = FString::Printf(TEXT("class '%s' is not a child of required base '%s'"),
			*Resolved->GetName(), *ClassProp->MetaClass->GetName());
		return;
	}
