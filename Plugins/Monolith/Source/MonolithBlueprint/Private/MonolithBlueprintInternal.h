...
namespace MonolithBlueprintInternal
{
	...
	inline TSharedPtr<FJsonObject> SerializePin(const UEdGraphPin* Pin)
	{
		...
		// Hidden pins are normally filtered out of nodes[].pins[] but are NOT
		// filtered out of export_graph's connections[]. Emitting the flag lets a
		// consumer tell "this edge references a pin I was not shown" from "this
		// payload is malformed". Only emitted when true, so existing readers are
		// unaffected.
		if (Pin->bHidden)
		{
		...
		// Two parallel edge lists. connected_to is the historical
		// "NodeName.PinName" form and stays first for back-compat; two pins with
		// the same name on one node are indistinguishable in it. connected_to_ids
		// carries the PinId GUID already emitted as "id" above, so an edge can be
		// resolved unambiguously. Indices correspond one-to-one.
		TArray<TSharedPtr<FJsonValue>> ConnArr;
		TArray<TSharedPtr<FJsonValue>> ConnIdArr;
		for (const UEdGraphPin* Linked : Pin->LinkedTo)
		{
			...
			ConnIdArr.Add(MakeShared<FJsonValueString>(Linked->PinId.ToString()));
		}
		PinObj->SetArrayField(TEXT("connected_to"), ConnArr);
		PinObj->SetArrayField(TEXT("connected_to_ids"), ConnIdArr);
		return PinObj;
	}
	...
	/**
	 * Serialize one node to JSON.
	 *
	 * @param bIncludeHiddenPins  Hidden pins are excluded by default, which is the
	 *        historical behaviour every existing caller expects. Pass true to emit
	 *        them (each carrying "hidden": true) when you need pins[] to account for
	 *        every edge in export_graph's connections[] -- self-pins on self-context
	 *        calls are the common case where the two disagree.
	 */
	inline TSharedPtr<FJsonObject> SerializeNode(UEdGraphNode* Node, bool bIncludeHiddenPins = false)
	{
		...
		{
			if (!Pin) continue;
			if (Pin->bHidden && !bIncludeHiddenPins) continue;
		...
		int32 MaxDepth = 100)
	{
		...
		FlowObj->SetStringField(TEXT("node"),
			Node->GetNodeTitle(ENodeTitleType::FullTitle).ToString());
		// Without this the trace is title-only and cannot be joined back to
		// get_graph_data's nodes[].id or export_graph's connections[].from_node,
		// which made it useless for verifying an edit landed.
		FlowObj->SetStringField(TEXT("node_id"), Node->GetName());
