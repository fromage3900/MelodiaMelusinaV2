...
void FMonolithBlueprintNodeActions::RegisterActions(FMonolithToolRegistry& Registry)
{
	...
	Registry.RegisterAction(TEXT("blueprint"), TEXT("set_node_property"),
		TEXT("Set a UPROPERTY on a graph node itself -- node-internal configuration that is neither a pin "
			"default nor a position. This is what binds an InputAction asset to a K2Node_EnhancedInputAction, "
			"and generally what nodes created through add_node's generic K2Node_ fallback need before they are "
			"usable. Path grammar matches set_property_at_path ('InputAction', 'NumInputs', 'Struct.Member[2]'). "
			"The node is reconstructed afterwards so its pins reflect the new value, and the response carries "
			"the re-serialized node plus pins_added / pins_removed -- read those, because ReconstructNode drops "
			"links silently when pin names change. "
			"REFUSED paths: NodeGuid and Pins (desynchronises the node from its graph); FunctionReference, "
			"EventReference, VariableReference and DelegateReference (use remove_node + add_node instead); "
			"NodePosX / NodePosY (use set_node_position)."),
		FMonolithActionHandler::CreateStatic(&HandleSetNodeProperty),
		FParamSchemaBuilder()
			.RequiredAssetPath(TEXT("asset_path"), TEXT("Blueprint asset path"))
			.Required(TEXT("node_id"),  TEXT("string"), TEXT("Node ID from get_graph_data / add_node"))
			.Required(TEXT("path"),     TEXT("string"), TEXT("Property path on the node, e.g. 'InputAction'"))
			.Required(TEXT("value"),    TEXT("any"),    TEXT("Value to write; coerced to the resolved leaf property type"))
			.Optional(TEXT("graph_name"), TEXT("string"), TEXT("Graph name (searches all graphs if omitted)"))
			.Optional(TEXT("reconstruct"), TEXT("bool"),
				TEXT("Reconstruct the node after writing so pins update. Default true. Setting false leaves pins "
					"possibly disagreeing with the property -- only useful to batch several writes before one reconstruct."),
				TEXT("true"))
			.Build());
...
{
	/**
	 * Head members that must never be written reflectively on a graph node.
	 *
	 * These are not conservatism -- each one produces a node that looks correct in
	 * the editor and is actually broken:
	 *   NodeGuid / Pins        : desynchronise the node from its owning graph.
	 *   *Reference members     : FMemberReference structs. Writing them in place
	 *                            bypasses the resolution add_node performs, so the
	 *                            node compiles against the wrong target.
	 *   NodePosX / NodePosY    : owned by set_node_position. Two writers over one
	 *                            field is the second-authority pattern this codebase
	 *                            avoids everywhere else.
	 */
	bool NodeActionsIsDeniedNodePropertyHead(const FString& Head, FString& OutReason)
	{
		struct FDenied { const TCHAR* Name; const TCHAR* Reason; };
		static const FDenied Denied[] = {
			{ TEXT("NodeGuid"), TEXT("NodeGuid identifies the node to its graph; rewriting it orphans references to this node") },
			{ TEXT("Pins"),     TEXT("Pins are owned by the node's own allocation; use connect_pins / set_pin_default instead") },
			{ TEXT("FunctionReference"), TEXT("use remove_node + add_node -- writing an FMemberReference in place skips add_node's resolution and yields a node that compiles against the wrong target") },
			{ TEXT("EventReference"),    TEXT("use remove_node + add_node (see FunctionReference)") },
			{ TEXT("VariableReference"), TEXT("use remove_node + add_node (see FunctionReference)") },
			{ TEXT("DelegateReference"), TEXT("use remove_node + add_node (see FunctionReference)") },
			{ TEXT("NodePosX"), TEXT("use set_node_position") },
			{ TEXT("NodePosY"), TEXT("use set_node_position") },
		};
		for (const FDenied& D : Denied)
		{
			if (Head.Equals(D.Name, ESearchCase::IgnoreCase))
			{
				OutReason = D.Reason;
				return true;
			}
	...
	void NodeActionsCollectPinNames(UEdGraphNode* Node, TArray<FString>& Out)
	{
		Out.Reset();
		if (!Node) return;
		for (UEdGraphPin* Pin : Node->Pins)
		{
			if (Pin) Out.Add(Pin->PinName.ToString());
		}
	}
}

FMonolithActionResult FMonolithBlueprintNodeActions::HandleSetNodeProperty(const TSharedPtr<FJsonObject>& Params)
{
	FString AssetPath;
	UBlueprint* BP = MonolithBlueprintInternal::LoadBlueprintFromParams(Params, AssetPath);
	if (!BP)
	...
	{
		return FMonolithActionResult::Error(FString::Printf(
			TEXT("Node not found: %s%s"), *NodeId,
			GraphName.IsEmpty() ? TEXT("") : *FString::Printf(TEXT(" (in graph '%s')"), *GraphName)));
	}

	// Head member = the path's first segment, before any '.' or '['.
	FString HeadMember = Path;
	{
		...
		if (DotIdx != INDEX_NONE && BracketIdx != INDEX_NONE) Cut = FMath::Min(DotIdx, BracketIdx);
		else if (DotIdx != INDEX_NONE) Cut = DotIdx;
		else if (BracketIdx != INDEX_NONE) Cut = BracketIdx;
		if (Cut != INDEX_NONE) HeadMember = Path.Left(Cut);
	}

	FString DenyReason;
	if (NodeActionsIsDeniedNodePropertyHead(HeadMember, DenyReason))
	{
		return FMonolithActionResult::Error(FString::Printf(
			TEXT("Refusing to write '%s' on a graph node: %s"), *HeadMember, *DenyReason));
	}
	...
	// --- Dry run: resolve against the live node but write only into scratch. ---
	if (DryRunGuard.IsDryRun())
	{
		FMonolithReflectionWalker::FPathResolveResult Resolved =
			FMonolithReflectionWalker::ResolvePath(NodeClass, Node, Path, /*bCreateMissingKeys=*/false);
		if (!Resolved.bOk)
		{
			return FMonolithActionResult::Error(FString::Printf(
				TEXT("path '%s' did not resolve on node class %s: %s"),
				*Path, *NodeClass->GetName(), *Resolved.Error));
		}
		...
		if (Result.Result.IsValid())
		{
			...
			Result.Result->SetStringField(TEXT("node_id"), NodeId);
			Result.Result->SetStringField(TEXT("node_class"), NodeClass->GetName());
		}
		return Result;
	}

	// --- Live write, in the engine edit cradle. ---
	TArray<FString> PinsBefore;
	NodeActionsCollectPinNames(Node, PinsBefore);

	UEdGraph* OwningGraph = Node->GetGraph();

	Node->SetFlags(RF_Transactional);
	FScopedTransaction Transaction(NSLOCTEXT("MonolithBlueprintNodeActions", "SetNodeProperty", "Monolith Set Node Property"));
	Node->Modify();
	...
	FProperty* HeadProp = NodeClass->FindPropertyByName(FName(*HeadMember));
	if (HeadProp)
	{
		Node->PreEditChange(HeadProp);
	}

	FDryRunReport Report;
	const FBulkFillFieldWrite W = FMonolithReflectionWalker::WriteLeaf(
		Resolved.LeafProp, Resolved.LeafPtr, JsonVal, Node, Spec, Report, Path);

	if (!W.bOk)
	{
		Transaction.Cancel();
		return FMonolithActionResult::Error(FString::Printf(
			TEXT("write to '%s' failed: %s"), *Path, *W.Reason));
	}

	if (HeadProp)
	{
		FPropertyChangedEvent Evt(HeadProp, EPropertyChangeType::ValueSet);
		Node->PostEditChangeProperty(Evt);
	}

	// Reconstruct so the pin set reflects the new configuration. This is the whole
	// point for nodes like K2Node_EnhancedInputAction, whose pins are derived from
	// the asset just written.
	...
	// StructurallyModified, not Modified: the pin set may have changed, and the
	// weaker call lets the edit silently revert on the next compile.
	FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(BP);

	TArray<FString> PinsAfter;
	NodeActionsCollectPinNames(Node, PinsAfter);

	TArray<TSharedPtr<FJsonValue>> AddedArr, RemovedArr;
	for (const FString& P : PinsAfter)
	{
		if (!PinsBefore.Contains(P)) AddedArr.Add(MakeShared<FJsonValueString>(P));
	}
	for (const FString& P : PinsBefore)
	{
		if (!PinsAfter.Contains(P)) RemovedArr.Add(MakeShared<FJsonValueString>(P));
	}

	TSharedPtr<FJsonObject> Root = MakeShared<FJsonObject>();
	Root->SetStringField(TEXT("asset_path"), AssetPath);
	Root->SetStringField(TEXT("graph_name"), OwningGraph ? OwningGraph->GetName() : FString());
	Root->SetStringField(TEXT("node_id"), NodeId);
	...
	Root->SetStringField(TEXT("previous_value"), W.CurrentValue);
	Root->SetBoolField(TEXT("applied"), true);
	Root->SetBoolField(TEXT("reconstructed"), bReconstructed);
	Root->SetNumberField(TEXT("pins_before"), PinsBefore.Num());
	Root->SetNumberField(TEXT("pins_after"), PinsAfter.Num());
	Root->SetArrayField(TEXT("pins_added"), AddedArr);
	Root->SetArrayField(TEXT("pins_removed"), RemovedArr);
	if (RemovedArr.Num() > 0)
	{
		Root->SetStringField(TEXT("warning"),
			TEXT("Pins were removed by ReconstructNode. Any links on those pins are gone -- "
				"UE drops them silently. Verify with assert_graph_matches against the pre-edit export."));
	}
	// The caller's next connect_pins depends on this pin set, so return the node
	// rather than making them issue a follow-up get_node_details.
	Root->SetObjectField(TEXT("node"), MonolithBlueprintInternal::SerializeNode(Node));
