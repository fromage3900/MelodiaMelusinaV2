...
**~114 Blueprint actions** via `blueprint_query()`. Discover first: `monolith_discover({ namespace: "blueprint" })` — that is the only authoritative count; the numbers in this file are indicative.
...
| `get_graph_summary` | `asset_path`, `graph_name` | Per-node id/class/title + exec targets, no data pins (~10 KB vs ~172 KB) |
| `get_graph_data` | `asset_path`, `graph_name`, `node_class_filter`? | Full topology: pins, connections, positions |
| `get_graph_fingerprint` | `asset_path`, `graph_name`, `mode`? | Stable structural hash — did this graph change? |
| `assert_graph_matches` | `asset_path`, `graph_name`, `spec`, `mode`? | Prove an expected subgraph is present (and forbidden nodes absent) |
...
Graph topology **is** readable. `get_graph_data` and `export_graph` both iterate the live
`UEdGraph`, so they reflect writes immediately. (The staleness warning in `MONOLITH_GUIDE.md`
applies to `project_query("get_asset_details")`, which serves an indexed snapshot — it does not
apply to these.)

Never trust a write call's `success` flag as proof. The loop:

```
export_graph              -> save it: rollback record AND assertion baseline
get_graph_fingerprint     -> before
<mutate>
compile_blueprint         -> not clean? STOP, do not continue a batch
assert_graph_matches      -> matched:false? STOP
get_graph_fingerprint     -> after; record both
save_asset
```

One asset per transaction. Never mutate a second Blueprint before asserting the first.

`assert_graph_matches` takes an `export_graph` payload as its `spec`, so authoring an assertion is:
wire it once, export, trim to the interesting nodes. Nodes match on a *semantic* key (function +
class, event name, variable name, macro, cast class) rather than the `K2Node_*_NN` instance name,
so an assertion survives deleting and re-adding an identical node. `spec.forbidden_nodes:
[{class, function}]` asserts absence — the assertion to use after replacing a node, to prove the
old one is really gone.

A failed assertion returns **success** with `matched:false`; branch on that field rather than
wrapping the call in error handling.
...
- Any `UK2Node_` subclass name works as `node_type` (generic fallback). Nodes made that way usually
  still need `set_node_property` to configure them — that is what the "may require additional
  configuration" warning means.
- `set_node_property` reconstructs the node afterwards. **Read `pins_removed` in the response:**
  `ReconstructNode` drops links on renamed pins silently.
