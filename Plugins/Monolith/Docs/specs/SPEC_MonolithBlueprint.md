...
**Verification loop (3 actions, 2026-07-31)**
| Action | Params | Description |
|--------|--------|-------------|
| `get_graph_fingerprint` | `asset_path`, `graph_name?`, `mode?` | Stable structural hash of a graph. Modes `topology` (default), `topology+defaults`, `full`. Node identity is **semantic** — `function`+`function_class`, `event_name`, `variable_name`, `macro_name`, `cast_class`, title fallback — never `GetName()`, whose `_NN` suffix changes on delete-and-re-add. Repeated keys are disambiguated by a *neighbourhood signature* (the node's own links expressed via neighbours' base keys), which is content-derived, so the hash is independent of node position and of instance naming. Explicitly excluded from every mode: instance names, `NodeGuid`s, comment nodes, pin ordering; positions are included only in `full`. SHA-1 over the sorted canonical line set. |
| `assert_graph_matches` | `asset_path`, `graph_name?`, `spec`, `mode?`, `ignore_defaults?` | Assert an expected subgraph is present. `spec` is an `export_graph` payload (or a trimmed subset), so no second schema exists. `mode` `subset` (default) requires spec nodes/connections to be present; `exact` also reports extras. `spec.forbidden_nodes: [{class, function}]` asserts **absence** — the assertion for proving a replaced node is gone. Connections resolve their `from_node`/`to_node` ids through `spec.nodes[]`, so a spec must carry the nodes its connections reference. Returns **Success** with `matched:false` on a miss (not an error) plus `missing_nodes`, `missing_connections`, `pin_default_mismatches`, `forbidden_nodes_present`, `extra_nodes`. |
| `set_node_property` | `asset_path`, `node_id`, `path`, `value`, `graph_name?`, `reconstruct?`, `dry_run?` | Write a UPROPERTY on the node object itself — node-internal config that is neither a pin default nor a position (binding an `InputAction` to a `K2Node_EnhancedInputAction`, switch/sequence pin counts, and generally what `add_node`'s generic `UK2Node_` fallback needs). Same dotted+bracket path grammar as `set_property_at_path`, same `FMonolithReflectionWalker` cradle, plus a node epilogue: `ReconstructNode()` → `NotifyNodeChanged` → `MarkBlueprintAsStructurallyModified` (structurally, because the pin set may have changed — `MarkBlueprintAsModified` lets the edit revert on the next compile). Returns the re-serialized node plus `pins_added`/`pins_removed`. **Denied head members** (each yields a node that looks right and compiles wrong): `NodeGuid`, `Pins`, `FunctionReference`, `EventReference`, `VariableReference`, `DelegateReference` (→ `remove_node` + `add_node`), `NodePosX`/`NodePosY` (→ `set_node_position`). |

**T3D injection (2 actions, 2026-08-08)**
| Action | Params | Description |
|--------|--------|-------------|
| `validate_nodes_t3d` | `asset_path`, `t3d`, `graph_name?`, `placeholders?` | Pre-flight a T3D payload against the reflection system **without touching any graph**. Resolves placeholders, then reports `findings[]` of `{severity, kind, detail, node}`. Kinds: `unresolved_node_class` (the `Begin Object Class=` path does not load), `unresolved_function` (a `MemberName` absent from its `MemberParent` — the common post-C++-rename failure), `unresolved_member_parent` (warning; function existence unchecked), `unresolved_placeholder` (a `{{token}}` never substituted, which would import as literal text), `malformed_t3d` (unbalanced `Begin`/`End Object`), `empty_payload` (no `Begin Object Class=` at all). Also reports `engine_accepts`, the verdict of UE's own `FEdGraphUtilities::CanImportNodesFromText`. Returns **Success** with `valid:false` on a miss, never an error. |
| `inject_nodes_t3d` | `asset_path`, `t3d`, `graph_name?`, `placeholders?`, `dry_run?`, `auto_layout?`, `position?`, `compile?` | Import nodes from raw T3D in **one atomic transaction**. `copy_nodes` already round-trips T3D between two live graphs; this accepts T3D that exists in no graph yet, which is what makes multi-node authoring a single call instead of a dozen `add_node`/`connect_pins` round trips. Order: resolve placeholders → reflection pre-flight (identical to `validate_nodes_t3d`) → `CanImportNodesFromText` → `ImportNodesFromText` inside an `FScopedTransaction`. **Any pre-flight error aborts before the graph is modified** — a payload that is half-valid leaves the graph byte-identical, not half-wired. Placeholder grammar: `{{GUID:name}}` mints one stable `FGuid` per distinct name for the call (reuse a name across a node's `NodeGuid` and another node's `LinkedTo` to preserve the link); `{{name}}` substitutes from `placeholders`. Post-import it forces unique `NodeGuid`s (colliding guids break `get_graph_fingerprint` and editor diffing) and offsets the cluster clear of existing nodes unless `position` is given. Returns a **compact** node summary (`id`, `class`, `title`, `x`, `y`, `linked_pins`) — not raw T3D; re-export with `export_graph` if you need the text. `compile` (default true) adds `compile_errors`/`compile_warnings`/`compiled_clean`. `nodes_injected != declared_nodes` emits a `warning`: UE silently drops nodes the target graph will not accept. Does **not** save the asset. |

> **Not idempotent.** Guid uniquing means a repeated injection produces a second working copy of the cluster, not a no-op. Delete first, or assert absence with `assert_graph_matches` + `spec.forbidden_nodes`, before re-running.

> **Read-fidelity fixes shipped alongside (2026-07-31).** Three long-standing gaps in the shared serializers, all additive:
> 1. **Hidden-pin symmetry.** `SerializeNode` filtered `Pin->bHidden` while `export_graph`'s `connections[]` did not, so an edge could reference a pin absent from `nodes[].pins[]` (self-pins on self-context calls are the common case). Pins now carry `"hidden": true`; `export_graph`/`get_graph_data` accept `include_hidden_pins` (default `false`, so existing callers are unchanged); connections carry `from_pin_hidden`/`to_pin_hidden`. Deliberately **not** fixed by filtering `connections[]` — that would delete real edges.
> 2. **Pin-ID addressing.** The `PinId` GUID was serialized as `id` but was not addressable. `SerializePin` now also emits `connected_to_ids[]` parallel to `connected_to[]`, and `export_graph` connections carry `from_pin_id`/`to_pin_id`, disambiguating two same-named pins on one node.
> 3. **`get_execution_flow` node ids.** The trace was title-only and could not be joined back to `get_graph_data`'s `nodes[].id`. Each flow object now carries `node_id` alongside the existing `node` title.
...
> **`TSubclassOf<>` writes fixed (2026-07-31).** `FMonolithReflectionWalker` had no `FClassProperty` branch. Because `FClassProperty` derives from `FObjectProperty`, every class-typed field fell through to `WriteObjectRef`, which called `StaticLoadObject(ObjProp->PropertyClass, …)` with `PropertyClass == UClass::StaticClass()` — and `/Game/Foo/BP_Bar` loads a `UBlueprint`, not a `UClass`, so the load always returned null. **Every `TSubclassOf` write through `set_cdo_property`, `set_property_at_path`, `set_cdo_properties` and `seed_data_asset` silently failed for Blueprint class paths.** `DispatchByPropertyType` now dispatches `FClassProperty` to a new `WriteClassRef` *before* the `FObjectProperty` branch (order matters — the derived check must come first). It accepts a generated-class path, a Blueprint asset path (appending `.<Leaf>_C`, the same retry `ResolveDefaultObjectForPin` already used pin-side), a `/Script/…` path, or a bare native name with A/U/F prefix tolerance, and enforces the `MetaClass` constraint before writing.

**Limitations / v1.1 follow-ups.**

- Soft refs (`PC_SoftObject` / `PC_SoftClass`) and `PC_Interface` defer to the existing primitives path — `(WISHLIST v1.1)` for first-class reflection-walker support. Note `FSoftClassProperty` derives from `FSoftObjectProperty` and still routes to `WriteSoftObjectRef`, which stores the path verbatim — pass the explicit `…_C` form for soft class fields until that gets the same treatment as `WriteClassRef`.
