...
{
    // Accept a full /Game/... asset path directly (Blueprints outside the default
    // /Game/Blueprints/ folder never resolved otherwise, and callers dereferenced
    // the resulting nullptr without checking -- crashed the editor).
    if (BlueprintName.StartsWith(TEXT("/Game/")))
    {
        if (UBlueprint* Found = LoadObject<UBlueprint>(nullptr, *BlueprintName))
        {
            return Found;
        }
    }
