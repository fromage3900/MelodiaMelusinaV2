...
{
    // The public direct bridge exposes the compact add_node/connect_nodes/
    // add_event_node vocabulary. Keep those aliases here so graph authoring
    // remains available even when the richer Monolith proxy is unavailable.
    if (CommandType == TEXT("connect_nodes"))
    {
        TSharedPtr<FJsonObject> AliasParams = MakeShared<FJsonObject>();
        AliasParams->SetStringField(TEXT("blueprint_name"), Params->GetStringField(TEXT("blueprint_name")));
        AliasParams->SetStringField(TEXT("source_node_id"), Params->GetStringField(TEXT("source_node_id")));
        AliasParams->SetStringField(TEXT("target_node_id"), Params->GetStringField(TEXT("target_node_id")));
        AliasParams->SetStringField(TEXT("source_pin"), Params->GetStringField(TEXT("source_pin_name")));
        AliasParams->SetStringField(TEXT("target_pin"), Params->GetStringField(TEXT("target_pin_name")));
        return HandleConnectBlueprintNodes(AliasParams);
    }
    else if (CommandType == TEXT("add_event_node"))
    {
        TSharedPtr<FJsonObject> AliasParams = MakeShared<FJsonObject>();
        AliasParams->SetStringField(TEXT("blueprint_name"), Params->GetStringField(TEXT("blueprint_name")));
        AliasParams->SetStringField(TEXT("event_name"), Params->GetStringField(TEXT("event_name")));
        TArray<TSharedPtr<FJsonValue>> Position;
        Position.Add(MakeShared<FJsonValueNumber>(Params->GetNumberField(TEXT("pos_x"))));
        Position.Add(MakeShared<FJsonValueNumber>(Params->GetNumberField(TEXT("pos_y"))));
        AliasParams->SetArrayField(TEXT("node_position"), Position);
        return HandleAddBlueprintEvent(AliasParams);
    }
    else if (CommandType == TEXT("add_node"))
    {
        ...
        {
            AliasParams->SetStringField(TEXT("event_name"), Params->GetStringField(TEXT("event_type")));
            return HandleAddBlueprintEvent(AliasParams);
        }
        if (NodeType.Equals(TEXT("VariableGet"), ESearchCase::IgnoreCase))
        {
            AliasParams->SetStringField(TEXT("component_name"), Params->GetStringField(TEXT("variable_name")));
            return HandleAddBlueprintGetSelfComponentReference(AliasParams);
        }
        if (NodeType.Equals(TEXT("Self"), ESearchCase::IgnoreCase))
        {
            return HandleAddBlueprintSelfReference(AliasParams);
        }
        if (NodeType.Equals(TEXT("CallFunction"), ESearchCase::IgnoreCase))
        {
            AliasParams->SetStringField(TEXT("function_name"), Params->GetStringField(TEXT("target_function")));
            if (Params->HasField(TEXT("target_class")) && !Params->GetStringField(TEXT("target_class")).IsEmpty())
            {
                AliasParams->SetStringField(TEXT("target"), Params->GetStringField(TEXT("target_class")));
            }
            return HandleAddBlueprintFunctionCall(AliasParams);
        }

        return FUnrealMCPCommonUtils::CreateErrorResponse(
            FString::Printf(TEXT("Direct add_node alias does not support node type '%s'"), *NodeType));
    }
