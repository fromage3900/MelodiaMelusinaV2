...
TOOLS = {
    "RunP0SmokeTest": {
        "desc": "Run the P0 PIE smoke harness and record a smoke gate.",
        "surface": "editor.run_pie_smoke + poll_pie_smoke",
        "monolith_action": "editor.run_pie_smoke",
        "guard": "sandbox map only",
    },
    "RunPerformanceCapture": {
        "desc": "Trigger a Monolith viewport capture + Unreal Insights trace for a region.",
        "surface": "editor.capture_scene_preview + editor.capture_anim_frames",
        "monolith_action": "editor.capture_scene_preview",
        "guard": "sandbox map only",
    },
    "AuditDataLayers": {
        "desc": "Enumerate Data Layers / sublevels in the current level and report active/assigned state.",
        "surface": "mesh.manage_sublevel + mesh.get_level_actors (read-only audit)",
        "monolith_action": "mesh.manage_sublevel",
        "guard": "read-only",
    },
    "CreateRhythmReactiveMaterialInstance": {
        ...
        "surface": "material.create_material_instance + set_instance_parameter",
        "monolith_action": "material.create_material_instance",
        "guard": "dry-run by default; owner confirmation for live creation",
    },
    "PlaceSpeedTreeBiomeTest": {
        "desc": "PCG/Mesh-scatter biome test using PRESENT SpeedTree assets (M_SpeedTreeMaster + wind reset).",
        "surface": "mesh.scatter_props / scatter_on_surface over SpeedTree assets",
        "monolith_action": "mesh.scatter_props",
    ...
    "ValidateWaterAuthority": {
        ...
        "surface": "blueprint.get_cdo_properties on UMelodiaWaterGameplaySubsystem",
        "monolith_action": "blueprint.get_cdo_properties",
        "guard": "read-only",
    },
    "ValidateMaraSkeleton": {
        ...
        "surface": "mesh.analyze_skeletal_mesh + animation.compare_skeletons",
        "monolith_action": "mesh.analyze_skeletal_mesh",
        "guard": "read-only",
    },
    "BakeHoudiniRegion": {
        "desc": "Bake a Houdini HDA region into UE (HDA_ENV_TerrainStamp/PathCorridor/ScatterMaskBuilder/HeroRockFamily/LOD_Collision_Batch) per MARA_P0_P3_HOUDINI_EXECUTION_PLAN. Bake-not-live-cook.",
        "surface": "HoudiniEngine asset bake -> /Game/Melodia/Environment/<Chapter>/Houdini/",
        "monolith_action": "(HoudiniEngine bake; no direct Monolith action)",
    ...
    "BuildHLODForRegion": {
        ...
        "surface": "mesh.setup_hlod",
        "monolith_action": "mesh.setup_hlod",
        "guard": "sandbox map only",
    },
}
