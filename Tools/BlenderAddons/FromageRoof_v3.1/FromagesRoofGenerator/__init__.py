...
def _build_geonode_roof(obj, p):
    ...
    if mod is None:
        mod = obj.modifiers.new(name="FromageRoof_GN", type='GEOMETRY_NODES')
...
class FROMAGE_ROOF_PT_main(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = box.row(align=True)
        row.scale_y = 1.3
        row.label(text="🎨 MATERIAL & BAKING", icon='SHADING')
        ...
        # ── ROOF MODIFIER STACK ───────────────────────────────────
        box = layout.box()
        box.label(text="📚 ROOF STACK", icon='STACK')
        col = box.column(align=True)
