"""AAA Concert Harp — Full Lookdev Build for Melusina.

Art direction: stylized storybook fantasy, warm wood, gold accents, nylon strings.
Proper concert harp proportions: ~1.8m tall, ~0.8m wide curve.

Features:
- Bezier curves for elegant S-curve neck
- Subdivision surface for smooth silhouettes
- Procedural wood grain with wave texture
- Metallic gold leaf accents on pillar and carvings
- 41 graduated nylon strings
- Studio 3-point lighting + HDRI ambient
- 4K beauty render with proper framing

Run: blender --factory-startup -b -P Tools/BlenderAddons/aaa_harp_melusina.py
"""
import bpy
import os
import math
...
print("=== AAA Concert Harp — Melusina Lookdev ===")
...
# Soundboard: tapered slab (the long base)
soundboard_pts = [
...
# Neck: sweeping S-curve from pillar top to soundboard tip
neck_pts = [
...
scene.render.filepath = os.path.join(out_dir, "harp_melusina_")
...
fbx_path = os.path.join(out_dir, "SM_Mus_Harp_Concert_Real_Melusina.fbx")
bpy.ops.object.select_all(action="DESELECT")
...
print("=== AAA Concert Harp — Melusina Lookdev Complete ===")
