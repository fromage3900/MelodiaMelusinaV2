"""AAA Concert Harp — Full Lookdev Polish in Blender 5.2.

Art direction: stylized storybook fantasy, warm wood, gold accents, nylon strings.
- Subdivision surface for smooth silhouettes
- Procedural wood grain with wave texture
- Metallic gold leaf accents
- Studio 3-point lighting + HDRI
- 4K beauty render

Run: blender --factory-startup -b -P Tools/BlenderAddons/aaa_harp_polished.py
"""
import bpy
import os
import math
...
print("=== AAA Concert Harp — Polished Lookdev ===")
...
def create_bezier_curve(name, points, location=(0,0,0), bevel_depth=0.03, bevel_res=6, extrude=0):
    """Create a bezier curve with proper topology."""
...
# Soundboard: elegant tapered arc (side profile)
soundboard_pts = [
...
sb = create_bezier_curve("Harp_Soundboard", soundboard_pts, bevel_depth=0.025, bevel_res=8, extrude=0.04)
...
pillar = create_bezier_curve("Harp_Pillar", pillar_pts, location=(0.12, 0.02, 0), bevel_depth=0.035, bevel_res=8)
...
neck = create_bezier_curve("Harp_Neck", neck_pts, location=(0.12, 0.02, 1.75), bevel_depth=0.028, bevel_res=8)

# Capitol (decorative top)
bpy.ops.mesh.primitive_uv_sphere_add(segments=24, ring_count=12, radius=0.035, location=(0.12, 0.02, 1.78))
capitol = bpy.context.active_object
capitol.name = "Harp_Capitol"
capitol.scale = (1.3, 1.0, 0.7)

# Base foot
bpy.ops.mesh.primitive_cylinder_add(vertices=24, radius=0.05, depth=0.03, location=(0.12, 0.02, -0.015))
foot = bpy.context.active_object
foot.name = "Harp_Foot"

# ============================================================
# 2. STRINGS (41 graduated, thin cylinders with subdivision)
# ============================================================
...
for i in range(41):
    ...
    string.modifiers["Subdivision"].levels = 2
    
    strings.append(string)

print(f"[strings] created {len(strings)} strings with subdivision")

# ============================================================
# 3. CONVERT CURVES TO MESH + SUBDIVISION
# ============================================================

curve_objects = [sb, pillar, neck]
for obj in curve_objects:
    bpy.context.view_layer.objects.active = obj
    obj.select_set(True)
    bpy.ops.object.convert(target="MESH")
    
    # Add subdivision surface for smooth silhouettes
    bpy.ops.object.modifier_add(type='SUBSURF')
    obj.modifiers["Subdivision"].levels = 2
    obj.modifiers["Subdivision"].render_levels = 3

# Subdivision for capitol and foot
...
# ============================================================
# 4. MATERIALS (Procedural AAA quality)
# ============================================================

def create_wood_material(name, base_color, grain_color, roughness=0.5):
    """Procedural wood with grain texture."""
...
# ============================================================
# 5. STUDIO LIGHTING (3-point + rim + ambient)
# ============================================================
...
fill.rotation_euler = (math.radians(30), math.radians(-15), math.radians(45))

# Rim light (back, silhouette separation)
...
harp_center = Vector((-0.05, 0.02, 0.8))
bpy.ops.object.camera_add(location=(0.5, -1.5, 0.9))
...
# ============================================================
# 7. RENDER SETTINGS (4K, Cycles, denoising)
# ============================================================
...
scene.render.filepath = os.path.join(out_dir, "harp_beauty_polished_")
scene.render.image_settings.file_format = "PNG"

# Background (warm parchment)
scene.world.node_tree.nodes[1].inputs[0].default_value = (0.88, 0.85, 0.78, 1.0)

print("[render] Cycles 4K, 1024 samples, OpenImageDenoise")
...
all_mesh = [sb, pillar, neck, capitol, foot] + strings
for obj in all_mesh:
    obj.select_set(True)
...
fbx_path = os.path.join(out_dir, "SM_Mus_Harp_Concert_Real_Polished.fbx")
bpy.ops.object.select_all(action="DESELECT")
...
print("=== AAA Concert Harp — Polished Lookdev Complete ===")
