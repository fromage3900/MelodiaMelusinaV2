...
def generate_world_chunks(midi_path, preset_id="walkable_valley",
    ...
    Yields WorldChunk objects. Uses shared core.field.build_field() to
    avoid duplicating the MIDI->field pipeline.
    """
    from .core.field import build_field, load_voxel_module
    import walkable_world as ww

    result = build_field(midi_path, preset_id, source="walkable")
    base_field = result["field"] if result["ok"] else None
        ...
        for cy in range(-view_distance, view_distance + 1):
            ...
            if base_field is not None:
                ...
                if xs and ys:
                    ...
                    yield chunk
                    continue
            # Fallback: per-chunk generate (keeps offline tests green)
