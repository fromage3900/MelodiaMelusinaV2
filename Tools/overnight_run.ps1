...
Invoke-PythonStep -Name "run_contract_tests" -Arguments @("Tools/run_contract_tests.py") -Required
...
Invoke-PythonStep -Name "health.py" -Arguments @("Tools/health.py", "--json") -Required
...
if (-not $SkipTier1) {
    # --- Tier 1: MATH/MCP harness ---
    Write-Log "--- Tier 1: MATH/MCP harness ---"
    Invoke-PythonStep -Name "run_math_models qwen" -Arguments @(
        "Tools/run_math_models.py", "--model", "qwen2.5-coder:7b", "--no-ter"
    ) -Required

    if (Test-OllamaModelPresent "muse-glimmer-30b") {
        Write-Log "muse-glimmer-30b found - running --run-muse"
        Invoke-PythonStep -Name "run_math_models muse" -Arguments @(
            "Tools/run_math_models.py", "--run-muse", "--no-ter"
        ) -Required
    }
    else {
        Write-Log "SKIP: muse-glimmer-30b not in ollama list"
    }
}
