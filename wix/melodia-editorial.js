...
(function (global) {
  ...
  function ensureThreeCss() {
    if (document.querySelector('link[data-melodia-three-css], link[href*="melodia-three.css"]')) {
      return;
    }
    var link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = 'melodia-three.css';
    link.setAttribute('data-melodia-three-css', '1');
    document.head.appendChild(link);
  }

  function bootThreeEffects(shell, starIntensity) {
    var wantsThree = hasEffect(shell, 'three');
    var hasMounts =
      !!document.querySelector('[data-three-orrery], [data-three-constellation], #three-constellation-mount');
    if (!wantsThree && !hasMounts) return;
    if (!global.MelodiaThreeCore) return;

    ensureThreeCss();

    // WebGL starfield supersedes 2D canvas when `three` is declared with starfield.
    if (wantsThree && hasEffect(shell, 'starfield') && global.MelodiaThreeStarfield) {
      global.MelodiaThreeStarfield.init({ intensity: starIntensity });
    }

    if (global.MelodiaThreeOrrery) {
      global.MelodiaThreeOrrery.mountAll();
    }
    if (global.MelodiaThreeConstellation) {
      global.MelodiaThreeConstellation.mountAll();
    }
  }
  ...
  function bootEffects() {
    ...
    const wantsThree = hasEffect(shell, 'three');

    // When Three.js owns the sky, skip the 2D starfield init here (bootThreeEffects handles it).
    if (!wantsThree && hasEffect(shell, 'starfield') && global.MelodiaStarfield) {
    ...
    bootThreeEffects(shell, starIntensity);
  }
