...
| **Cymatics / audio→geometry Chladni** | **PRESENT — source-verified, read-only MPC consumer** | `Source/.../MelodiaCymaticsSubsystem.h/.cpp` (`UGameInstanceSubsystem`, `FTSTicker`, reads `MPC_Melodia_Palette` `BeatPulse`/`BassIntensity` read-only via `GetParameterCollectionInstance` — no writes, `IsReadOnlyByContract()=true`, Chladni `cos(n·π·u)cos(m·π·v)−cos(m·π·u)cos(n·π·v)`) · `Tools/Houdini/copernicus_cymatic_parallax.py` · `Saved/Audit/copernicus_cymatic/` **53 variants × 2617 PNGs (293 Height, 9 maps: BaseColor/Normal/Roughness/Metallic/Height/ORM/Emissive/Iridescence/Opacity, ~250 MB, all PNG magic valid; 22 variants ingested to `Content/EnvSandbox/Textures/Copernicus/` 2026-09-02)** · probe `Saved/Audit/cymatics_probe_2026-08-31.json` 5/5 PASS · audit `Saved/Audit/cymatics_audit_2026-09-01.json`. Build: closed-editor `Build.bat` required for PIE. Single-writer guard: `UMelodiaAudioReactivePresentationSubsystem` remains the sole MPC writer — this subsystem is READ-ONLY. Promoted from §2 SCAFFOLDED 2026-09-01 (grand_review phase_1). Monolith lane Phase A for new variants defined in `Tools/monolith_execution_plan_parallel.py` (53 variants, dry-run 271→~600 steps) — execute `--phase A --variant <new> --execute` when editor idle to create `/Game/Melodia/Cymatics/` materials. |
