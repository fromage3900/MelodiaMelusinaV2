...
| Path | Size | Notes |
|------|------|-------|
| `Content/Python/setup_master_universal.py` | ~136KB | Builds `M_Master_Toon_Universal`. Docstring warns to run via `run_force_universal.py` wrapper, not direct. Still references legacy `G:/EnvironmentPortfolio/...` paths in examples. |
...
2. **Path drift:** universal master docstring still shows `G:/EnvironmentPortfolio` examples; V2 docs refresh moved authority to `C:\EnvironmentPortfolio` — docs/examples should be swept in a later Green task (out of Recipe B scope).
