# Career Docs

> **Downstream of the game.** This is marketing / funding / hiring material for
> **Melodia Melusina**, a single-person AAA-tier UE 5.8 rhythm-JRPG. It exists to fund and staff
> the game. **No agent may cite anything in this folder as project direction** — authority is
> [`../../../PROJECT.md`](../../../PROJECT.md).

Home for role targeting, studio research, resume bullets, and application tracking.

**Lead with the game.** Every draft here should open with **Melodia Melusina** — a single-person
AAA-tier UE 5.8 rhythm-JRPG — and treat the pipeline as the credibility argument behind it, not
as the product. The previous framing ("the Melodia Platform", a pipeline-infrastructure company)
inverted that and has been corrected; see
[MELODIA_STUDIO_STARTUP_PARAGRAPH.md](MELODIA_STUDIO_STARTUP_PARAGRAPH.md).

### Active sendoffs (refreshed 2026-09-05)

**Canonical application ledger / send queue:** [RECRUITER_SENDOFFS_2026-08-25.md](RECRUITER_SENDOFFS_2026-08-25.md)  
**Current weekly scout + senior-framing plan:** [WEEKLY_ROLE_SCOUT_AND_SENIOR_FRAMING_PLAN_2026-09-05.md](WEEKLY_ROLE_SCOUT_AND_SENIOR_FRAMING_PLAN_2026-09-05.md)

| # | Target | Status |
|---|--------|--------|
| 1 | SageMesh — Synthetic Data / 3D Technical Artist | **APPLY FIRST** |
| 2 | 9 Story / Brown Bag — Unreal Environment Generalist | **APPLY** |
| 3 | Hardsuit Labs — Technical Artist | Targeted stretch |
| 4 | Rockstar Toronto — Procedural Technical Artist | Stretch / career target |
| 5 | Marks — Creative Technologist CGI | Opportunistic |
| 6 | Certain Affinity — Senior Material Artist | Lower priority; shipped-title gap |
| 7 | Blue Isle — Senior Environment Artist | Reference / stretch |
| — | Older OpenCode / Velan / Infold / Nous queue | Retained in canonical ledger |
| — | NVIDIA Toronto | **WITHDRAWN** — archive only |

Older draft bodies also live in [EMAIL_DRAFTS_2026-08-20.md](EMAIL_DRAFTS_2026-08-20.md) (priority table updated).

### Two things to never get wrong in an outgoing draft

1. **Do not cite the MATH benchmark table.** The 100-task / 98.8% TCA figures were **withdrawn
   2026-08-19** and were never measured. The only real evaluation artifact is `eval_results.json`
   (one model, one task, 60.35%, 0% pass) — and even that run was invalidated by a harness
   defect. Cite the gate ledger and the two genuine local-model probe results instead.
2. **"Playable" is a gated claim.** The four shipping gates have PASS rows; the six orchestra
   convergence gates do not. Check `python Tools/echo_run.py status` before writing a status
   sentence, and say which you mean.

## Active application drafts

| Studio | File | Status |
|---|---|---|
| **SageMesh / 9 Story / Hardsuit / Rockstar / Marks / Certain Affinity / Blue Isle** | [WEEKLY_ROLE_SCOUT_AND_SENIOR_FRAMING_PLAN_2026-09-05.md](WEEKLY_ROLE_SCOUT_AND_SENIOR_FRAMING_PLAN_2026-09-05.md) · [RECRUITER_SENDOFFS_2026-08-25.md](RECRUITER_SENDOFFS_2026-08-25.md) | **Current live queue** |
| OpenCode / Velan / Infold / Nous legacy queue | [RECRUITER_SENDOFFS_2026-08-25.md](RECRUITER_SENDOFFS_2026-08-25.md) | Retained; below Sep 5 queue |
| Epic MegaGrants (funding, not a job) | [EPIC_MEGAGRANTS_APPLICATION_DRAFT.md](EPIC_MEGAGRANTS_APPLICATION_DRAFT.md) | Draft — needs real numbers/URLs filled in |
| Promethean AI | [STUDIO_PROMETHEAN_AI_DRAFT.md](STUDIO_PROMETHEAN_AI_DRAFT.md) | Draft — strong fit, Junior Artist (remote) |
| Digital Extremes | [STUDIO_DIGITAL_EXTREMES_DRAFT.md](STUDIO_DIGITAL_EXTREMES_DRAFT.md) | Draft — good fit, pick one experience level honestly |
| Infold Games | [STUDIO_INFOLD_GAMES_DRAFT.md](STUDIO_INFOLD_GAMES_DRAFT.md) | Draft — campus track preferred; see sendoffs |
| Compulsion Games | [STUDIO_COMPULSION_GAMES_DRAFT.md](STUDIO_COMPULSION_GAMES_DRAFT.md) | Watchlist — closest requirement-match found, but no open posting confirmed; check Canadian work-auth eligibility first |
| NVIDIA Toronto | [STUDIO_NVIDIA_DRAFT.md](STUDIO_NVIDIA_DRAFT.md) · [NVIDIA_DEVREL_PACKET…](NVIDIA_DEVREL_PACKET_2026-08-20.md) | **WITHDRAWN** — archive |

Each studio draft has real, web-verified company info (dated), an honest fit assessment including gaps/mismatches (not just a pitch), a draft cover-letter opening to rewrite in your own voice, and a "before you submit" checklist. None of these are ready to send as-is — brackets need filling, and the "before you submit" items are real blockers, not formalities.
