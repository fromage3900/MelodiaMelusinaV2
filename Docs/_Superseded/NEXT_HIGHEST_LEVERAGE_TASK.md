...
> Pipeline hardening SSOT and agent handoff prompt: [Docs/ASSET_PIPELINE_HARDENING_AND_AGENT_PROMPT_2026-07-14.md](../ASSET_PIPELINE_HARDENING_AND_AGENT_PROMPT_2026-07-14.md).
...
## Today GÇö 2026-07-11 (monetization + site)
...
1. **Ornament store screenshots (6)** GÇö highest revenue leverage; FBX 15/15 + preview ZIP already on disk.
2. **Re-pack sell ZIP from current SurrealArch KitbashExport FBXs** GÇö preview ZIP is undersized vs live meshes.
3. **Gumroad create + upload** GåÆ then flip `store_live` + gh-pages.
4. **Website honesty pass** GÇö shop gates, Stage passport proof, Props Mini as next SKU (this session).
5. **Claude songcraft GåÆ C++ resolve** GÇö core gameplay loop (parallel; not a store SKU).
6. **Pillar hero captures** GÇö still the top *portfolio landscape* lever when the editor is free (P0 below).

## What changed 2026-07-09 (context for ranking)

The four WP pillar levels are now **actually** production-ready GÇö WP=true, per-pillar displaced terrain (483GÇô685cm relief), PCG verified live: SakuraDream 2015 / SpaceCathedral 642 / BaroqueGrotto 1085 / CosmicOrrery 6171 instances, all saved. Four root-cause bugs fixed (WP detection, `clear_graph_nodes` no-op, SurfaceSampler-on-mesh GåÆ MeshSampler variants, squared perlin frequency). `M_Master_Toon_Universal` deep-reviewed: **healthy** GÇö the 916GåÆ1015 expression growth is 32 gated, default-off feature commits; the "26 broken texture refs" are ~23 validator false positives (TextureObject-pin-fed) + 3 real-but-dormant empty samplers; the duplicate switch trios are benign hygiene debt. MelodiaCore plugin (source-only, unbuilt) was killing every unattended boot GÇö disabled in the .uproject.

## P0 GÇö Directly gates the portfolio (Infinity Nikki Aesthetic)

1. **Hero captures on the 4 pillars.** Snap `Cam_Hero_Establishing` to real content per level (terrain now has actual relief and scatter aligned with Nikki style genomes like `NikkiFlorawish` and `NikkiStoneville`). Run `render_exporter.py --all-heroes` to capture the dream-palette landscapes, then website ingest (`my-site-clean/tools/ingest_unreal_portfolio.ps1` + `validate_portfolio.ps1`). This is THE task GÇö everything else feeds it.
2. **Reparent `MI_NikkiHero_*`** (scratch master GåÆ `M_Master_Toon_Universal`) and **fix `MI_Show_CelestialNebula` BSS texture refs** GÇö both appear in pillar hero frames. Verify each via `capture_material_grid`; save with `only_if_is_dirty=False` + disk-timestamp check.
...
## P1 GÇö Melodia Studio + GMM foundation

5. **Lock the shared family contract.** Implement the pure-Python `gmm.family` scaffold, versioned manifest fixture, role/style registry, provenance shape, and schema validator described in [Docs/MELODIA_GMM_FAMILY_ARCHITECTURE_PLAN.md](../MELODIA_GMM_FAMILY_ARCHITECTURE_PLAN.md). This is the highest-leverage bridge between Blender authoring and UE integration.
...
8. **Verify the Blender GåÆ UE handoff.** Export one manifest and world artifact from Melodia Studio, import it in GMM/UE, and compare bounds/counts/material hints in a deterministic test stage. Live Link is optional feedback, not the source of truth.
...
## P1 GÇö Ecosystem & Styling Hygiene (Phase 1 of the unification plan)

5. **Masters/ folder cleanup + retire `M_Master_Toon_Unified` / `M_Master_Simple_Universal`** (reparent instances GåÆ verify GåÆ archive). 28 assets sit in Masters/, only ~7 are masters; `M_SpaceParallax_Test` is still on disk despite the 07-09 "deleted" changelog claim.
...
7. **Collapse duplicate switch trios** (`bWeather_Active`+ù3, `bCelestialUsesDreamPalette`+ù3 GÇö one switch node feeding 3 consumers each) and **fix or delete `TextureSample_0/1/2`** (dormant black-out if `bTriplanar` ever flips on).
8. **Fix `validate_material` false positives** (skip samples whose TextureObject input pin is connected GÇö Monolith C++; remember the Live Coding patch doesn't persist across editor restarts) GÇö then re-run the library audit. The standing "72 missing refs" figure predates all of this and must be re-measured before anyone acts on it.

## P2 GÇö PCG Library Truthing & Genome Alignment (Phase 2)

9. **Graph validation gate** (`Saved/Audit/pcg_graph_health.json`): per graph GÇö node-count sanity (no duplicate chains), every spawner has GëÑ1 mesh entry, generate-on-test-stage ISM within declared band. Ensure compliance with the 8 Infinity Nikki genome presets in 4 families (`NikkiFlorawish`, `NikkiStoneville`, `NikkiHeartcraft`, `NikkiMirage`).
...
## P3 GÇö Parked (needs human-at-keyboard or source reading)
...
- PCGEx Collections stagingGåÆselector contract (needs PCGEx source read) GÇö or delete `PCG/Collections/`.
- Blender surreal-architecture generator GåÆ UE world.json/HISM bridge end-to-end verification (biggest untapped scale lever for the cathedral/Escher goal).
...
- `L_SakuraPath` art pass is human-owned. No lights/cameras spawned or screenshots taken in user levels GÇö data verification only.
